...
struct ExecInner {
    ...
    send_queue: UnsafeCell<Option<(Sender<NonNull<()>>, Receiver<NonNull<()>>)>>,
}
impl ExecInner {
    ...
    unsafe fn receiver(&self) -> Option<&Receiver<NonNull<()>>> {
        let send_queue = &*self.send_queue.get();
        let receiver = send_queue.as_ref().map(|(_, receiver)| receiver);
        receiver
    }
}
pub struct ExecSend {
...
impl Exec {
    ...
    pub fn send_handle(&self) -> SendHandle {
        unsafe {
            ...
            let (sender, _) = send_queue.get_or_insert_with(|| bounded(100_000));
            SendHandle {
                sender: sender.clone(),
            }
        }
        ...
        unsafe {
            ...
            if let Some(receiver) = receiver {
                ...
                while let Ok(val) = receiver.try_recv() {
                    let rc = ptr_to_rc(val);
                    let value =
                        &mut *(&rc.executor as *const *const ExecInner as *mut *const ExecInner);
                    *value = self.inner.as_ref() as *const ExecInner;
                    std::mem::forget(rc);
                    poll_queue.push_back(val);
                }
            }
...
impl SendHandle {
    pub fn send<T: Future<Output = ()> + Send + 'static>(&self, f: T) {
        unsafe {
            self.sender
                .send(create_future_obj(std::ptr::null(), f))
